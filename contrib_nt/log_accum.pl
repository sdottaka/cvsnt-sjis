# $Id: log_accum.pl,v 1.4 2003/02/07 17:45:09 tmh Exp $
...
#
# Hacked even more for NT by Tony Hoyle

use Win32;

# usage: log_accum.pl [-d] [-s] [-m mailto] ...] -i $(CVSPID) -r $(CVSROOT) -u $(USER) %s
#	-d		- turn on debugging
...
#	-i		- (Win32: Required) $CVSPID - PID of parent
#	-r		- (Win32: Required) $CVSROOT - Root of repository
#	-u		- (Win32: Required) $USER - Real User
#
#	Win32 doesn't come with a mailer.  'blat' is suggested.
...
$MAILER	       = "blat - -server mail -q -s \"CVS commit notification\"";

# Change to match commit_prep.pl

$LAST_FILE     = "c:/temp/#cvs.lastdir";

$CHANGED_FILE  = "c:/temp/#cvs.files.changed";
$ADDED_FILE    = "c:/temp/#cvs.files.added";
$REMOVED_FILE  = "c:/temp/#cvs.files.removed";
$LOG_FILE      = "c:/temp/#cvs.files.log";
...
sub cleanup_tmpfiles {
    ...
    $wd = Win32::GetCwd();
    chdir("c:/temp") || die("Can't chdir('c:/temp')\n");
    opendir(DIR, ".");
...
sub mail_notification {
    ...
    open(MAIL, "| $MAILER -f $login\@magenta-logic.com -t $mailto -s \"CVS update: " . $modulename . "\"") || die ("Couldn't open mailer");
...
#$domainname = ".magenta-logic.com";

#$hostdomain = $hostname . $domainname;
$hostdomain="magenta-logic.com";
...
($id) || die("Must supply id on command line (-i)\n");
($cvsroot) || die("Must supply cvsroot on command line (-r)\n");
if ($replyto eq '') {
    $replyto = $login;
}
...
if ($_ ne $cvsroot . "/" . $files[0]) {
    if ($debug) {
	print STDERR sprintf("Current directory %s is not\n last directory %s.\n", $cvsroot . "/" .$files[0], $_);
    }
    exit 0;
}
if ($debug) {
...
for ($i = 0; ; $i++) {
	...
	foreach $dofile (@changed_files) {
	    ...
	    open(STATUS,"cvs -nQq status -v $dofile |") || die ("Couldn't start cvs\n");
