...
static void find_identical_ends PARAMS((struct file_data[], int));
  ...
  while ((char const *) p < suffix_begin)
   ...
   hashing_done:;
      ...
      if (line == alloc_lines)
	{
	  ...
	  linbuf = (char const **) xrealloc ((void*)(linbuf + linbuf_base),
...
static void
find_identical_ends (filevec,binary)
	 ...
	 int binary;
{
  ...
  slurp (&filevec[0], binary);
  if (filevec[0].desc != filevec[1].desc)
    slurp (&filevec[1], binary);
  else
    {
  ...
  if (! (no_diff_means_no_output
      ...
      while (p0 != end0)
	{
	  ...
	  if (l == alloc_lines0)
	    linbuf0 = (char const **) xrealloc ((void*)linbuf0, (alloc_lines0 *= 2)
							 * sizeof(*linbuf0));
	  linbuf0[l] = p0;
	  while (*p0++ != '\n')
	    ;
     ...
     int pretend_binary;
{
  ...
  find_identical_ends (filevec, appears_binary);
