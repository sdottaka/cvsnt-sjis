    ...
    char **argv;
{
	...
	if(is_rcs)
	{
		...
		for(n=0; n<argc; n++)
		{
			...
			finfo.update_dir = xstrdup(finfo.fullname);
			finfo.update_dir[(name-finfo.fullname)-1]='\0';
			...
			if(finfo.rcs)
			{
				err += rcs_update_fileproc(&finfo,options, tag, date,
							...
							pipeout, 0, join_rev1, join_rev2, 0);
...
static void build_one_dir (char *repository, char *dirpath, int sticky)
{
    FILE *fp;
...
static int checkout_proc (
    int argc, char **argv, char *where_orig, char *mwhere, char *mfile,
    int shorten, int local_specified, char *omodule, char *msg)
{
    ...
    if (! verify_read(mapped_repository,tag))
    {
		if(tag)
	       ...
	       error (0, 0, "User %s cannot access %s", CVS_Username, fn_root(argv[0]));
       return (1);
    }
	...
	if (pathncmp (repository, current_parsed_root->directory,
...
internal error: %s doesn't start with %s in checkout_proc",
	...
	while (cp > where)
	{
	    ...
	    if (pathcmp (reposcopy, current_parsed_root->directory) == 0)
	    {
...
static char *findslash (char *start, char *p)
{
    ...
    {
#ifdef SJIS
		if (ISDIRSEPMB(start,p)) return p;
#else
