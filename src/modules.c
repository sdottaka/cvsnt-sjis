 ...
 */
int do_module (DBM *db, char *mname, enum mtype m_type, char *msg, CALLBACKPROC callback_proc,
...
module `%s' is a request for a file in a module which is not a directory",
  ...
  do_special:
	    ...
	    (m_type == mtCHECKIN && checkin_prog != NULL))
	{
	    ...
	    char *real_prog = NULL;
	    char *prog;
	    char *real_mwhere = (where != NULL ? where : mwhere);
	    char *expanded_path;
	    ...
	    if ((*prog != '/') && (*prog != '.'))
	    {
		real_prog = xmalloc (strlen (real_mwhere) + strlen (prog)
				     + 10);
