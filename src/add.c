    ...
    char **argv;
{
    ...
    if (! verify_create (repository, vers->tag))
	{
		if(vers->tag)
			error (1, 0, "User '%s' cannot change %s on tag/branch %s", CVS_Username, repository, vers->tag);
		else
    ...
    char *tag;
{
    ...
    Register (entries, user, "0", line, options, tag, (char *) 0, (char *) 0, NULL, NULL);
    xfree (line);
    return (0);
}
