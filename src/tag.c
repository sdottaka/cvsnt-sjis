...
static const char rtag_opts[] = "+AabdFBflnQqRr:D:";
...
static const char tag_opts[] = "+AbcdFBflQqRr:D:M";
...
static int rtag_proc (
    int argc, char **argv, char *xwhere, char *mwhere, char *mfile, int shorten,
    int local_specified, char *mname, char *msg)
{
    ...
    err = start_recursion (is_rtag ? rtag_fileproc : tag_fileproc,
			   ...
			   local, which, 0, 0, where, 1, verify_write);
    Lock_Cleanup ();
	xfree(current_date);
    dellist (&mtlist);
    if (where != NULL)
...
static int pretag_proc(const char *repository, const char *filter)
{
	...
	TRACE(1,"pretag_proc(%s,%s)",PATCH_NULL(repository),PATCH_NULL(filter));
	...
	if(isinfolibrary(filter))
	{
	    ...
	    walklist(tlist, pretag_list_proc, (void*)1);
		if(!cb->pretag)
		{
			error(0,0,"Pretag function missing in %s - cannot call",filter);
			ret = 0;
		}
		else
			ret = cb->pretag(cb,symtag,delete_flag?"del":force_tag_move?"mov":"add",fn_root(repository),pretag_list_count,pretag_list);
		xfree(pretag_list);
	...
	if(taginfo_uses_stdin)
	{
		cmd=(char*)xmalloc(strlen(filter)+strlen(symtag)+10+strlen(repository)+256);
		sprintf(cmd,"%s %s %s %s",filter,symtag,delete_flag?"del":force_tag_move?"mov":"add",fn_root(repository));
    ...
    walklist(tlist, pretag_list_proc, pipefp);
...
static int pretag_list_proc(Node *p, void *closure)
{
    if (p->data != NULL)
    {
		...
		else if(taginfo_uses_stdin)
		{
			char file[8192];
			shell_escape(file,p->key);
			fprintf((FILE*)closure,"%s %s\n",file,p->data);
		}
		else
		{
			run_arg(p->key);
    ...
    struct file_info *finfo;
{
     ...
     */
	if(floating_branch && !alias_branch)
		sprintf(strrchr(version,'.'),".0");
