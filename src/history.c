 ...
 */
static int historyinfo_proc (const char *repository, const char *filter)
{
	char *cmd;
	FILE *pipefp;
	...
	if(isinfolibrary(filter))
	{
		...
		if(!cb->history)
		{
			error(0,0,"History function missing in %s - cannot call",filter);
			ret = 0;
		}
		else
			ret = cb->history(cb,fn_root(repository),history_line);
	...
	cmd=(char*)xmalloc(strlen(filter)+strlen(repository)+256);
	sprintf(cmd,"%s %s",filter,fn_root(repository));
	if ((pipefp = run_popen (cmd)) == NULL)
	{
		if (!noexec)
			error (0, 0, "cannot write entry to history filter: %s", filter);
		xfree(cmd);
		return (1);
	}
	fprintf(pipefp,"%s",history_line);
    ...
    char *repository;
{
	...
	real_workdir = xstrdup(fn_root(workdir));
	...
	sprintf (line, "%c|%24.24s|%s|%s|%s|%s|%s\n",
	     type, ctime(&now),
	     username, real_workdir, repos, revs, name);

	history_line = line;
	if (Parse_Info (CVSROOTADM_HISTORYINFO, repos, historyinfo_proc, 1) > 0)
	{
...
#ifdef TIME_64BIT
#ifdef _WIN32
	sscanf(line,"%I64x",&hr->date);
	cp=line;
	while(*cp && (isdigit(*cp) || (tolower(*cp)>='a' && tolower(*cp)<='f')))
		cp++;
#else
    ...
    char *fname;
{
    ...
    if ((fd = CVS_OPEN (fname, O_RDONLY | OPEN_BINARY,0)) < 0)
    ...
    struct hrec *hr;
{
	...
	{
	    if (hr->mod && !pathcmp (hr->mod, *cpp))	/* found module */
		break;
	}
	if (!count)
