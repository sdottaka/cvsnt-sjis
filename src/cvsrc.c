...
read_cvsrc (int *argc, char ***argv, char *cmdname)
{
    ...
    char *line = NULL, *server_line = NULL, *dummy_line = NULL;
    int line_length;
	...
	if (server_started)
	{
		if(supported_request("read-cvsrc2")) /* Sensible version */
		{
			...
			send_to_server ("read-cvsrc2\012", 0);
		...
		else if(supported_request("read-cvsrc")) /* Version done after one too many beers */
		{
			...
			send_to_server ("read-cvsrc\012", 0);
			buffer = xmalloc(1);
			len = 0;
			do
			{
				...
				if(!strcmp(server_line,"end-cvsrc"))
					break;
				buffer = xrealloc(buffer, len+strlen(server_line)+2);
