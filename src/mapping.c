...
static int modules2_module_struct_sort(const void *_a, const void *_b)
{
	...
	return pathcmp(a->directory[0]=='*'?&a->directory[1]:a->directory,b->directory[0]=='*'?&b->directory[1]:b->directory);
}
	...
	while(fgets(buf,sizeof(buf), f))
	{
		...
		*(p+1)='\0';
		...
		else
		{
			...
			q=p;
			inquote=0;
			line->directory=xmalloc(strlen(p)+1);
			line->translation=xmalloc(strlen(p)+1);
			line->regex=NULL;
...
static modules2_module_struct *lookup_repository_directory(modules2_struct *module, const char *directory, int *partial_match, int *subdir_match)
{
	...
	dir = bsearch(&term,module->module,module->module_count,sizeof(modules2_module_struct),modules2_module_struct_sort);
	if(dir)
	{
...
int regex_filename_match(const char *regex, const char *filename)
{
	...
	/* A Null regex matches '*' */
	if(!regex)
		return 1;

	if(filenames_case_insensitive)
		regex_err = regcomp(&reg, regex, REG_ICASE|REG_EXTENDED|REG_NOSUB);
	else
		regex_err = regcomp(&reg, regex, REG_EXTENDED|REG_NOSUB);
	if (regex_err)
	{
		char buf[1024];
	...
	regex_err = regexec(&reg, filename, 0, NULL, 0);
	regfree(&reg);
	TRACE(3,"Match %s to %s = %d",PATCH_NULL(regex),PATCH_NULL(filename),regex_err?0:1);
	return regex_err?0:1;
}
	...
	if(!remote)
	{
		...
		if(current_directory->repository_rcsfile)
		{
			...
			if(!rev)
				rev=xstrdup("1.1");
			if(!nonbranch && tag)
				RCS_settag(current_directory->repository_rcsfile,tag,rev,NULL);
			...
			/* Temporarily close the buffer... we'll reopen it later */
			rcsbuf_close(&current_directory->repository_rcsfile->rcsbuf);
		}
	}

	tmp = xmalloc(strlen(dir)+strlen(CVSADM_RENAME)+256);
	if(dir && dir[0])
	{
		sprintf(tmp,"%s/",dir);
		pt=tmp+strlen(tmp);
...
static int create_mapping_file(const char *repository, const char *message)
{
	if(!current_directory->repository_rcsfile)
	{
		char *fn = xmalloc(strlen(repository)+strlen(RCSREPOVERSION)+10);
		TRACE(3,"Creating new directory mapping file");
		sprintf(fn,"%s/%s%s",repository,RCSREPOVERSION,RCSEXT);
		add_rcs_file(message?(char*)message:"created",fn,NULL,"1.1",NULL,NULL,NULL,0,NULL,"mapping file",12,NULL,NULL);
		xfree(fn);
		current_directory->repository_rcsfile = RCS_parse(RCSREPOVERSION,repository);
		if(!current_directory->repository_rcsfile)
...
int commit_directory(const char *update_dir, const char *repository, const char *message)
{
	...
	if(RCS_checkin(current_directory->repository_rcsfile,NULL,message?(char*)message:"no message",NULL,0,NULL,NULL,directory_commitproc, &newver))
...
{
	char *ren = xmalloc(strlen(directory)+sizeof(CVSADM_RENAME)+20);
	char *rennew = xmalloc(strlen(directory)+sizeof(CVSADM_RENAME)+20);
	char *of = xmalloc(MAX_PATH+strlen(oldfile));
	FILE *fpin,*fpout;
...
int upgrade_entries(const char *repository, const char *dir, List **entries, List **renamed_files)
{
	...
	directory_data *old_directory = directory_stack+directory_stack_size-2;
...
#ifdef SERVER_SUPPORT
	/* If the client doesn't support the "Renamed" response then this just becomes
	   a simple search for potentially renamed file */
	if(server_active && !client_can_rename())
		client_supports_rename = 0;
#endif
	...
	*renamed_files = getlist();
			...
			if(!strcmp(from,oldrepos))
			{
				...
				if(client_supports_rename)
				{
					if(to && *to)
					{
						...
						xfrom = xmalloc(strlen(dir)+strlen(from)+10);
						sprintf(xfrom,"%s/%s",dir,last_component(from));
						xto = xmalloc(strlen(dir)+strlen(to)+10);
	...
	{
		char *log = xmalloc(strlen(dir)+sizeof(CVSADM_ENTLOG)+10);
		FILE *f;
