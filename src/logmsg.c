...
static int editinfo_proc(const char *repository, const char *template);
...
static int array_proc (Node *p, void *closure)
{
    ...
    if (li->type == type)
    {
		...
		loginfo_array[pos].type=li->type;
	}
	return 0;
}
...
static int fmt_proc (Node *p, void *closure)
{
	...
	{
		char *tmp = xmalloc(strlen(p->key)*4);
		(void) fprintf (fp, "%s ", shell_escape(tmp,p->key));
		col += strlen (tmp) + 1;
		xfree(tmp);
	}
    }
...
do_editor (char *dir, char **messagep, char *repository, List *changes)
{
  ...
  "%s----------------------------------------------------------------------\n",
    ...
    run_arg (fname);
    if ((retcode = run_exec (RUN_NORMAL)) != 0)
 ...
 */
void
do_verify (char **message, const char *repository)
{
    FILE *fp;
    char *fname;
    int retcode = 0;
	...
	if (repository != NULL)
	    Parse_Info(CVSROOTADM_VERIFYMSG, (char*)repository, 
			       verifymsg_proc, 0);
	...
	if (verifymsg_script)
	{
		if(isinfolibrary(verifymsg_script))
		{
			library_callback *cb = open_infolibrary(verifymsg_script);
			if(!cb)
			{
				error(0, 0, "Can't open info library: %s",verifymsg_script);
			}
			if(!cb->verifymsg)
			{
				error(0,0,"Precommit function missing in %s - cannot call",verifymsg_script);
			}
			else
			{
				if(cb->verifymsg(cb,fname))
...
title_proc (Node *p, void *closure)
{
	...
	escaped_file = xmalloc(strlen(p->key)*2);
	shell_escape(escaped_file,p->key);
 ...
 * filter program.
 */
static int logfile_write (const char *repository, const char *filter, const char *message, FILE *logfp, List *changes)
{
    FILE *pipefp;
    char *prog;
    char *cp;
	...
	*cp++ = '"';
	...
	if(isinfolibrary(prog))
	{
		...
		setup_arrays("",changes);
		if(!cb->loginfo)
		{
			error(0,0,"Loginfo function missing in %s - cannot call",filter);
			ret = 0;
		}
		else
 ...
 */
/* ARGSUSED */
static int editinfo_proc(const char *repository, const char *editor)
{
