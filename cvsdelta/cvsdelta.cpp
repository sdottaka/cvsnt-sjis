 ...
 * cvsdelta
 * 
 * Binary diff algorythm.  Some bits from xdelta (mostly checksum/hash stuff),
...
unsigned cvsdelta::match(const Byte *p1, const Byte *p2, const Byte **pbase, int max_back, int max_forward)
{
  ...
  while((*(--ul1)==*(--ul2)) && back<max_back-3)
   back+=4; 
  p1=(const Byte *)(ul1+1);
  p2=(const Byte *)(ul2+1); 
  while((*(--p1)==*(--p2)) && back<max_back)
    back++;
  *pbase = p1+1;
  ...
  while(*(ul1++)==*(ul2++) && forward<max_forward-3)
	forward+=4;

  p1=(const Byte *)(ul1-1);
  p2=(const Byte *)(ul2-1); 
  while(*(p1++)==*(p2++) && forward<max_forward)
	  forward++;
  *p1--;
  *p2--;
  return back+forward;
