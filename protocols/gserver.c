...
static void gserver_destroy(const struct protocol_interface *protocol);
...
#ifdef GSS_AD
static int gserver_impersonate(const struct protocol_interface *protocol, const char *username, void *user_handle);
#endif
...
struct protocol_interface *get_protocol_interface(const struct server_interface *server)
{
	...
	return &gserver_protocol_interface;
}

void gserver_destroy(const struct protocol_interface *protocol)
{
		...
		*(short*)buf=len;
		...
		if (got < 0)
		    server_error (1, "receive from server %s: %s",
			   current_server->current_root->hostname, gai_strerror(socket_errno));
		buf[got + 2] = '\0';
		if (buf[got + 1] == '\n')
		    buf[got + 1] = '\0';
...
void gserver_error(int error, OM_uint32 stat_min, OM_uint32 stat_maj, const char *msg)
{
	...
	if(stat_maj)
	{
		...
		server_error (stat_min?0:error, "%s: %s", msg, (char *) tok_out.value);
	}
	...
	if(stat_min)
	{
		...
		gss_display_status (&new_stat_min, stat_min, GSS_C_MECH_CODE,
					GSS_C_NULL_OID, &message_context, &tok_out);
		server_error (error, "%s: %s", msg, (char *) tok_out.value);
	}
}
...
static int gssapi_wrap(int encrypt, const void *buffer, int size, void *output, int *newsize)
{
    ...
    if ((stat_maj = gss_wrap (&stat_min, gcontext, encrypt, GSS_C_QOP_DEFAULT, &inbuf, &conf, &outbuf)) != GSS_S_COMPLETE)
	{
...
#ifdef GSS_AD
int gserver_impersonate(const struct protocol_interface *protocol, const char *username, void *user_handle)
{
