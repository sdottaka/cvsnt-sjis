...
#define AppVersion0 GetFileVersion("\dev\cvsnt-sjis\bin\cvs.exe")
...
Name: Server\LsaHelper; Description: Setuid LSA ヘルパー; Types: custom full typical; MinVersion: 0,5.0.2195; Check: isadmin
...
Filename: {sys}\net.exe; Parameters: start cvs; Tasks: service; Components: Server; WorkingDir: {app}; Description: CVS サービスを開始する; StatusMsg: CVS サービスを開始します; Flags: runhidden postinstall runascurrentuser nowait
Filename: {sys}\net.exe; Parameters: start cvslock; Components: Server; Tasks: lockservice; WorkingDir: {app}; Description: CVS ロックサービスを開始する; StatusMsg: CVS ロックサービスを開始しています; Flags: runhidden postinstall runascurrentuser nowait
...
begin
	...
	if IsAdminLoggedOn and (CurStep = ssInstall) then
	begin
...
var rslt : integer;
begin
	Exec(GetSystemDir()+'\net.exe','stop cvs',GetSystemDir(),SW_SHOW,ewWaitUntilTerminated,rslt);
	Exec(GetSystemDir()+'\net.exe','stop cvsssh',GetSystemDir(),SW_SHOW,ewWaitUntilTerminated,rslt);
	Result := True;
end;
...
var rslt : integer;
begin
	Exec(GetSystemDir()+'\net.exe','stop cvslock',GetSystemDir(),SW_SHOW,ewWaitUntilTerminated,rslt);
	Result := True;
end;
