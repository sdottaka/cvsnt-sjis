...
bool DoModified(SOCKET s,size_t client, char *param)
{
...
#ifdef SJIS
	id = (char *)_mbschr((unsigned char *)param,'|');
#else
	id = strchr(param,'|');
#endif
	if(!id)
	...
	lockId = helper;
#ifdef SJIS
	branch = (char *)_mbschr((unsigned char *)id,'|');
#else
	branch = strchr(id,'|');
#endif
	if(!branch)
	...
	*(branch++)='\0';
#ifdef SJIS
	version = (char *)_mbschr((unsigned char *)branch,'|');
#else
	...
	*(version++)='\0';
#ifdef SJIS
	oldversion = (char *)_mbschr((unsigned char *)version,'|');
	if(_mbschr((unsigned char *)oldversion+1,'|'))
#else
...
#ifdef SJIS
	branch = (char *)_mbschr((unsigned char *)param,'|');
#else
	branch = strchr(param,'|');
#endif
	if(!branch)
